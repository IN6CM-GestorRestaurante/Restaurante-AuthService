...
public class MongoSyncService : ISyncService
{
    ...
    public async Task SyncUserToMongoAsync(User user)
    {
        try
        {
            ...
            // Change BaseURL to /branch/v1
            await _httpClient.PostAsync("http://server-admin:3001/branch/v1/users/sync", jsonContent);
            // Also fallbacks to localhost if server-admin is not resolvable natively
            await _httpClient.PostAsync("http://localhost:3001/branch/v1/users/sync", jsonContent);
        }
        catch (Exception ex)
        {
